function [ TAP, APs, lists ] = classification(ims, method, K, T, k, s )
...
% OUTPUTS
%   TAP   :  Total Average Precision
%   APs   :  A 1x4 array with the Average Precision per class
%   lists :  A cell array. Each column corresponds to the ranked list per
%            class (eg lists(:,1) for class "airplanes")

E      =  50; % number of test images per class (max 50)
flag   =  1; % controls whether to use built-in kmeans of matlab or vl_kmeans
%              for building the visual dictionary
%           0 for kmeans (too slow and causes memory issues for K = 2000)
%           1 for vl_kmeans
...
% K-MEANS CLUSTERING
fprintf('Building visual dictionary of %d visual words...\n\n', K);
...
fprintf('%d random images per class (excluding previously used)...\n', T);

fprintf('Processing airplanes_train...\n');
h1 = getHists(ims, T, '../airplanes_train', M, method, s);
...
switch method
    case 1
        [~, descriptors] = vl_sift(single(image));
    case 2
...
for i = 1:quantity % quantity represents the number of images for testing
    im1 = myRead(images(i, :), s); 
    des = mySift(im1, method);
    % alittletrick to avoid time consuming for loops
    dd = sum(des'.^2, 2);
    dm = des' * Means';
    mm = sum(Means.^2, 2)';
    dists = sqrt(bsxfun(@plus, mm, bsxfun(@minus, dd, 2*dm)));
